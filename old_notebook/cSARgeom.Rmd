...
library(dplyr)
library(purrr)
data=read_excel("coa_pa99_centroides.xlsx")
```

##Generate and plot points and minimum convex polygon
```{r}
points = st_as_sf(data,coords=c("long","lat"), crs=3763)
...
A function that picks a random point and starts sampling increasing circles from those points, returning the points inside those circles. 
...
test the function
```{r}
pt_in_circles = filter_points_in_expanding_circles(points,2000*1:10,convex_hull)
...
```{r}
#habitat names shoudl be a df with a col with habitat codes and respective habitat names
...
##create_squares
Create an sf where each sampling point is associated with a sampling square centred in the point and with a width dx.
```{r}
...
Filter points for type II/III SAR (fractals sampling scheme or sub-divisions), Points should have an associated sampling square as geometry feature instead of a point geometry.
```{r}
...
habitat_values <- c(1,2,3,NA)
species_group_names = c("Sp_Forest", "Sp_Other")

classif<-read_excel("coa_aves_classification_v2.xlsx")
...
res=summarize_samples(flatten(samples), flatten(polygons),
                lu1995, habitat_names, habitat_values, species_groups,
